# Memory management
sysctl -w vm.swappiness=10                # 最小化交换空间使用
sysctl -w vm.overcommit_memory=1          # 严格分配内存，防止过度分配
sysctl -w vm.overcommit_ratio=90          # 允许使用90%的物理内存
sysctl -w vm.dirty_ratio=40                # 增加脏页的最大比例，允许更多脏页在内存中
sysctl -w vm.dirty_background_ratio=20      # 增加脏页背景写入比例
sysctl -w vm.dirty_expire_centisecs=2000   # 縮短脏页保持时间，快速写入
sysctl -w vm.dirty_writeback_centisecs=2500 # 快速写回脏页
sysctl -w vm.vfs_cache_pressure=20         # 保持更多文件系统缓存
sysctl -w vm.min_free_kbytes=524288       # 提高最小可用内存，防止内存不足
sysctl -w vm.zone_reclaim_mode=0          # 关闭区域回收模式

# Network settings
sysctl -w net.core.somaxconn=4096         # 提高最大连接数
...
sysctl -w net.core.netdev_max_backlog=300000 # 提高最大排队长度
sysctl -w net.ipv4.tcp_rmem="4096 8388608 16777216" # 增加TCP接收缓冲区
sysctl -w net.ipv4.tcp_wmem="4096 8388608 16777216" # 增加TCP发送缓冲区
...
sysctl -w net.ipv4.tcp_fin_timeout=10     # 缩短FIN超时时间
sysctl -w net.ipv4.tcp_max_tw_buckets=300000 # 增加最大TIME_WAIT桶数
sysctl -w net.ipv4.tcp_low_latency=1      # 优化TCP低延迟传输
sysctl -w net.ipv4.tcp_syn_retries=2      # 减少SYN重试次数
sysctl -w net.ipv4.tcp_congestion_control=hybla # 使用Hybla算法
